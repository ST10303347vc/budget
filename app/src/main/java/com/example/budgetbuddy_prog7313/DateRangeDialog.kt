...
fun DateRangeDialog(
    ...
    if (openFrom.value) {
        ...
        DatePickerDialog(
            context,
            { _, y, m, d -> tempFrom = "%04d-%02d-%02d".format(y, m + 1, d); openFrom.value = false },
    ...
    if (openTo.value) {
        ...
        DatePickerDialog(
            context,
            { _, y, m, d -> tempTo = "%04d-%02d-%02d".format(y, m + 1, d); openTo.value = false },
            cal.get(Calendar.YEAR), cal.get(Calendar.MONTH), cal.get(Calendar.DAY_OF_MONTH)
        ).show()
    }
    ...
    AlertDialog(
        ...
        confirmButton = {
            TextButton(onClick = {
                if (tempFrom.isNotBlank() && tempTo.isNotBlank()) {
                    println("Filtering from $tempFrom to $tempTo")
        ...
        text = {
            Column {
                Button(onClick = { openFrom.value = true }, modifier = Modifier.fillMaxWidth()) {
                    Text(if (tempFrom.isBlank()) "Pick From Date" else "From: $tempFrom")
                }
                Spacer(modifier = Modifier.height(10.dp))
                Button(onClick = { openTo.value = true }, modifier = Modifier.fillMaxWidth()) {
